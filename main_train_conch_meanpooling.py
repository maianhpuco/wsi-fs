...
def prepare_dataset(args, fold_id):
    if args.dataset_name == 'tcga_renal':
        ...
        print(f"[✓] Loaded Fold {fold_id} - Train: {len(train_dataset)} | Val: {len(val_dataset)} | Test: {len(test_dataset)}")
...
def main(args):
    ...
    for i in range(args.k_start, args.k_end + 1):
        ...
        args.results_dir = os.path.join(args.paths['results_dir'], f"results_fold{i}_{timestamp}")
        ...
        config.weight_path = args.paths.get('weight_path', 'hf_hub:MahmoodLab/conch')

        model = CONCH_ZeroShot_Model_MeanPooling(config=config, num_classes=args.n_classes).to(args.device)
