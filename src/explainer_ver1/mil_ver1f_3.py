...
class ViLa_MIL_Model(nn.Module):
    ...
    def forward(self, x_s, coord_s, x_l, coords_l, label):
        ...
        text_enhancement_low = text_enhanced_features_low.mean(0, keepdim=True).expand_as(compents)
        ...
        # Enhance with properly encoded text prototypes
        text_enhancement_high = text_enhanced_features_high.mean(0, keepdim=True).expand_as(compents_high)
