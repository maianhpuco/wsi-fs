...
class Ver2f(nn.Module):
    def __init__(self, config, num_classes=None):
        ...
        self.all_desc = sum(self.text_prompts.values(), [])
    ...
    def forward(self, x_s, coord_s, x_l, coord_l, label=None, contrastive_weight=0.1):
        ...
        ce_loss = self.loss_ce(logits, label)
        ...
        total_loss = ce_loss + contrastive_weight * contrastive_loss_total
    ...
    def get_slide_desc_summary(self, patch_feats, threshold=0.25):
        ...
        for b in range(B):
            ...
            for class_id, head in enumerate(self.desc_heads):
                ...
                desc_names = self.all_desc[desc_start:desc_end]

                for j in range(attn_weights.shape[1]):
                    if (attn_weights[:, j] > threshold).any():
                        slide_desc_set.add(desc_names[j])
            summary_list.append(sorted(slide_desc_set))
        return summary_list
