...
class DescriptionHead(nn.Module):
    def __init__(self, desc_feats):
        ...
        self.scale = desc_feats.shape[1] ** -0.5  # scaling for dot-product attention

    def forward(self, patch_feats):
        """
        Args:
            patch_feats: [B, N, D] - patch-level features
        Returns:
            scores: [B, N] - similarity scores between each patch and this class
        """
        B, N, D = patch_feats.shape
        num_desc = self.desc_feats.shape[0]

        # [B, N, D] x [D, num_desc] → [B, N, num_desc]
        sim = torch.matmul(patch_feats, self.desc_feats.T) * self.scale
        attn_weights = F.softmax(sim, dim=-1)  # attention over description prompts

        # Compute weighted sum over description embeddings
        # [B, N, num_desc] x [num_desc, D] → [B, N, D]
        weighted_desc = torch.matmul(attn_weights, self.desc_feats)

        # Final similarity score between patch and description set
        scores = (patch_feats * weighted_desc).sum(dim=-1)  # [B, N]

        return scores.unsqueeze(-1)  # [B, N, 1]
