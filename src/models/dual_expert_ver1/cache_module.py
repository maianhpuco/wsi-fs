...
class VQGANCacheModule(nn.Module):
    ...
    def build_memory_bank(self, k_shot_features, k_shot_labels, class_prompts, clip_text_encoder, vqgan_encoder):
        ...
        for c in range(self.n_classes):
            ...
            self.memory_bank['class_embeddings'][start_idx:start_idx + self.prototypes_per_class] = clip_text_encoder(class_prompts[c])
            self.memory_bank['importance_scores'][start_idx:start_idx + self.prototypes_per_class] = importance[top_indices]
            
            # Token prompts are initialized randomly and learned during training

    def augment_features(self, features, word_indices):
        """Augment features with learnable token context prompts."""
        # Map word indices to token prompts
    ...
    def retrieve(self, test_features, predicted_class_emb, vqgan_encoder):
        ...
        # Augment test features with token prompts
    ...
    def forward(self, features, labels, class_prompts, clip_text_encoder, vqgan_encoder):
        """Training forward pass."""
        # Build memory bank if not already built
        if not self.memory_bank['visual_words'].sum():  # Check if empty
            self.build_memory_bank(features, labels, class_prompts, clip_text_encoder, vqgan_encoder)
        
        # Retrieve cache logits and augmented features
        predicted_class_emb = clip_text_encoder(class_prompts[labels])  # Placeholder for training
        cache_logits, augmented_features = self.retrieve(features, predicted_class_emb, vqgan_encoder)
